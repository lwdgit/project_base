...
function ipMaskMergeOk (ip, mask) {
	var ipArr = ip.split('.'),
		...
		mergeRslt,
		i,
		msg = _("IP Address and Mask doesn't match!");
	for(i = 0; i < 4; i++) {
...
$.dealWithJSonDate = function (url, fnSuccess, flag){
	$.ajax({
		url: url,
		type: 'get',
		dataType: 'json',
		success:  fnSuccess,
		error: function (req) {
			var errData = req.responseText;
			if (errData.indexOf("<!DOCTYPE") !== -1) {
				window.location.reload();
			}
			//console.log(errData);
			jsonObj = {};
			// Delete control characters
			dataStr = errData.replace(/[\x00-\x1F\x7F]/g, "");
...
var progressStrip = (function(window) {
	function ProgressStrip(url, reboot_time, reboot_msg,  up_time, up_msg) {
		...
		this.up_timer = null;
		this.up_msg = up_msg ||
					  _("The system is flashing now. DO NOT POWER OFF THE DEVICE!");
		this.up_pc = 0;
		this.reboot_pc = 0;
